...
class DeepSeekAI:
    def __init__(self, api_key: str, base_url: str, model: str,
        ...
        # 检查是否为 Ollama 服务
        if '/api/chat' in base_url or 'ollama' in base_url:
            self.is_ollama = True
    ...
    def get_response(self, message: str, user_id: str, system_prompt: str) -> str:
        ...
        try:
            ...
            # 检查是否是 Ollama API
            if self.is_ollama:
                # Ollama API 格式 - 直接使用原始消息，不包含系统提示词和对话历史
                request_config = {
                    "model": self.config["model"].split('/')[-1],  # 移除路径前缀
                    "messages": [{
                        "role": "user",
                        "content": message  # 直接使用原始消息
                    }],
                    "stream": False,
                    "options": {
                ...
                # 使用 requests 库向 Ollama API 发送请求
                try:
    ...
    def get_ollama_models(self) -> List[Dict]:
        """获取 Ollama 可用的模型列表"""
        try:
            # 使用配置的 base_url 的域名和端口部分
            base_url = str(self.client.base_url)
            # 从 base_url 中提取域名和端口，处理不同的URL格式
            if '/api/' in base_url:
